...
const Posts = () => {
    ...
    const getDoctor = async () => {
        if (initdoctors) {
            await axios.get("https://mydoctor-burhanpur.herokuapp.com/doctors").then(doctor => setFrist(doctor.data)).catch(err => console.log(err))
        }
    }
    ...
    const findDoc = async () => {
        setinitDoc(false);
        if (search.length > 0) {
            await axios.post('https://mydoctor-burhanpur.herokuapp.com/doctors/search', { search: search }).then(doc => setDoc(doc.data)).catch(err => console.log("Error -> " + err))
